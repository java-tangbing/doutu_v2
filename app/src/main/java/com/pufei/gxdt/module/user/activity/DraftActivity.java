...
public class DraftActivity extends BaseMvpActivity<DraftPresenter> implements DraftView {
    ...
    TextView tv_title;
    @BindView(R.id.draft_xryv)
    XRecyclerView rl_publish;
    @BindView(R.id.fragment_draft_smart)
    ...
    public void initView() {
        ...
        jokeAdapter = new JokeAdapter(DraftActivity.this,jokeList);
        ...
        jokeAdapter.setOnItemClickListener(new JokeAdapter.MyItemClickListener() {
            ...
            public void setOnItemClickListener(View itemview, View view, int postion) {
                if (jokeList.get(postion).getType()==0){
                    try {
                        Intent intent = new Intent(DraftActivity.this, JokeDetailActivity.class);
...
//                        startActivity(intent);
            ...
            public void OnBtDelete(int position) {
                Toast.makeText(DraftActivity.this, "已收藏", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    @Override
    public void getData() {
        if (NetWorkUtil.isNetworkConnected(DraftActivity.this)) {
            requestJoke(page);
        }else{
...
//            presenter.getPublish(RetrofitFactory.getRequestBody(jsonObject.toString()));
    ...
    @Override
    public void resultDraft(PictureResultBean bean) {
        if (page == 1) {
            jokeList.clear();
        }
//        jokeList.addAll(bean.getResult());
    ...
    @Override
    public void setPresenter(DraftPresenter presenter) {
        if (presenter == null) {
